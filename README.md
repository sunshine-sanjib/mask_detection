😷 Face Mask Detection System
📌 Project Overview
The Face Mask Detection System is a computer vision–based application designed to automatically detect whether a person is wearing a face mask or not in real time. The system uses deep learning and transfer learning techniques to achieve accurate and efficient classification from both static images and live video streams.
This project leverages the MobileNetV2 pre-trained architecture for feature extraction along with a custom classification head for binary classification (Mask / No Mask). Due to its lightweight architecture, MobileNetV2 enables real-time performance even on systems with limited computational resources.
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
🎯 Project Objectives
Detect face masks accurately in real time
Utilize transfer learning for faster training and better performance
Build a lightweight and efficient deep learning model
Demonstrate practical use of computer vision in public safety
🧠 Model Architecture
base Model: MobileNetV2 (Pre-trained on ImageNet)
Approach: Transfer Learning
Classification: Binary (Mask / No Mask)
Framework: TensorFlow & Keras
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
🛠️ Tech Stack
Programming Language: Python
Libraries & Frameworks:
TensorFlow
Keras
OpenCV
NumPy
Matplotlib
Tools & Platforms:
Google Colab
Jupyter Notebook
GitHub

---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
📂 Project Structure
Face-Mask-Detection/
│
├── dataset/
│   ├── with_mask/
│   └── without_mask/
│
├── model/
│   └── mask_detector.model
│
├── notebooks/
│   └── training_and_testing.ipynb
│
├── face_mask_detection.py
├── requirements.txt
├── README.md
└── sample_output.png
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
👤 Author
G SANJIB
GitHub: https://github.com/sunshine-sanjib
LinkedIn: www.linkedin.com/in/g-sanjib-26s
📜 License
This project is licensed under the MIT License.
⭐ Portfolio Note
Built a real-time Face Mask Detection System using MobileNetV2 and transfer learning, capable of accurate mask classification from images and live video streams.
